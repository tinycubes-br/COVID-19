...
typedef struct __attribute__((__packed__)) {
    ...
    PBinNode last;
    struct {
        __uint64_t   count    :33;         // up to 128G-1
        unsigned int bin_size :13;         // up to 128K-1
...
alocate_in_last_node:
        ...
        //printf("BINLIST: Nao Primeiro. N=%d\n", n);

        for (i=n, pbi = p->items; i; i--, j++) {
        ...
        // se chegou aqui, então nao tem mais espaco
        //printf("Espaço esgotado.\n");
        #define MAX_ITEMS_PER_NODE  10
        #define COMPUTE_NODE_SIZE(N) (sizeof(BinNode) + (item_size * (N)))

//        int node_size = sizeof(BinNode) + item_size;
        static int map_n_items[MAX_ITEMS_PER_NODE+1]  = { 1, 1, 2, 3, 4, 6, 6, 8, 8, 10, 10 };

        // verifica se pode crescer
        if (n+1 >= MAX_ITEMS_PER_NODE) {
            //printf("Nao pode crescer P=%p\n",p);
        } else {
            int old_node_size = COMPUTE_NODE_SIZE(n);
            n = map_n_items[n+1];
            int node_size = COMPUTE_NODE_SIZE(n);
            //printf("Relocando... P=%p  Last=%p, LastNItems=%d N=%d Old=%d\n",p, pbl->last,pbl->data.last_node_n_items,n,old_node_size);
            
            // aloca novo node
            PBinNode new = calloc(1,node_size);
    ...
    //printf("BINLIST: new id_bin %d  %ld  %p\n", id_bin, value, pbl);
    //printf("Alocando... P=%p  Last=%p, LastNItems=%d\n",p, pbl->last,pbl->data.last_node_n_items);
    ...
    pbl->last = p;
    count ++;
    //if (count > 1000000) fatal("finished");
...
static void _shallow_copy(PByte dst, PByte src, int n_bytes) {    
    ...
    */
    ...
    for (p = s->first; p; p = p->next_bin) {
        int n = (p != s->last) ? MAX_ITEMS_PER_NODE : s->data.last_node_n_items;
        int items_size = item_size * n;
        int node_size = sizeof(bin_t) + items_size;
#ifdef __USE_POOL__
